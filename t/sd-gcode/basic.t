...
BEGIN {
    plan skip_all => "Google sync broken";
...
( $ret, $out, $err ) =
  run_script( 'sd',
    [ 'clone', '--from', $sd_gcode_url, '--non-interactive' ] );
my $for_sd_id;
...
( $ret, $out, $err ) =
  run_script( 'sd', [ 'attachment', 'content', $att_id ] );
diag($err) if ($err);
...
run_script( 'sd',
    [ 'ticket', 'comment', $for_sd_id, '-m', 'comment from sd' ] );
