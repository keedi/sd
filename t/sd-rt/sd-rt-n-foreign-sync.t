...
$CF->AddValue(
    ...
    Description =>
      'Using this flag will forbid syncing the ticket to other foreign replicas',
);
$CF->AddValue(
    Name => 'AllowForeignSync',
    Description =>
      'Using this flag will allow syncing the ticket to other foreign replicas',
);
...
my $ticket = RT::Client::REST::Ticket->new(
    ...
    status  => 'new',
    subject => 'Fly Man',
    cf      => { 'sd:no-foreign-sync' => 'ForbidForeignSync', },
...
as_bob {
    ...
    ( $ret, $out, $err ) =
      run_script( 'sd',
        [ 'pull', '--from', repo_uri_for('alice'), '--force' ] );
...
TODO: {
    ...
    ok(
        !$task->load_by_cols( summary => "Fly Man" ),
        "no 'Fly Man' ticket on HM because RT had the no-foreign-sync custom field"
    );
}
