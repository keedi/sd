...
my $refuge = RT::Test->load_or_create_queue( Name => 'Ticket Refuge', );

my $root = RT::Client::REST->new( server => $url );
$root->login( username => 'root', password => 'password' );

diag(
    "create a ticket as root, then try to pull it as someone who doesn't have the rights to see it"
);
...
as_alice {
    ...
    ( $ret, $out, $err ) =
      run_script( 'sd', [ 'pull', '--from', $alice_rt_url, '--force' ] );
    ok($ret);
...
as_alice {
    ( $ret, $out, $err ) =
      run_script( 'sd', [ 'pull', '--from', $alice_rt_url ] );
    ok($ret);
...
as_alice {
    run_output_matches(
        'sd',
        [ 'ticket', 'update', $flyman_id, '--', 'priority=20' ],
        [qr/ticket .*$flyman_id.* updated/i],
    );

    ( $ret, $out, $err ) =
      run_script( 'sd', [ 'push', '--to', $alice_rt_url ] );
    ok($ret);
