...
my ( $ticket_id, $ticket_uuid ) =
  create_ticket_ok( '--summary', 'YATTA', '--owner', 'foo@bar.com' );
my ( $ticket_id_2, $ticket_uuid_2 ) =
  create_ticket_ok( '--summary', 'huzzah!', '--owner', 'alpha@bravo.org' );

diag('default -- no sorting, no grouping');
run_output_matches(
    'sd',
    [ 'ticket',            'list' ],
    [ qr/(\d+) YATTA new/, qr/(\d+) huzzah! new/, ]
);

diag('using --sort owner');
run_output_matches(
    'sd',
    [ 'ticket',              'list', '--sort', 'owner' ],
    [ qr/(\d+) huzzah! new/, qr/(\d+) YATTA new/, ]
);
