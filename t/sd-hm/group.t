...
{
    my $task = BTDT::Model::Task->new( current_user => $GOODUSER );
    $task->create( summary => "without the group", );
    diag $task->id;
}
...
my ( $flyman_uuid, $flyman_id );
{
    run_output_matches(
        'sd',
        [ 'ticket', 'list', '--regex', '.' ],
        [qr/(.*?)(?{ $flyman_uuid = $1 }) Fly Man (.*)/]
    );
    ( $ret, $out, $err ) =
      run_script( 'sd',
        [ 'ticket', 'show', '--batch', '--id', $flyman_uuid ] );
...
# pull
{
  ...
  TODO: {
        local $TODO = ' Investigate changeset count. Why do we have 2 extra?';
...
{
    my ( $yatta_id, $yatta_uuid ) =
      create_ticket_ok( '--summary', 'BLABLA', '--status', 'new' );
