...
ok( $ticket->update( summary => 'The product does not contain a pony' ),
    "updated!" );
unlike( $ticket->summary, qr/moose/,
    "The summary does not look like a moose" );
...
run_output_matches( 'sd', [ 'attachment', 'content', $att_id ],
    [qr/TIMTOWTDI/] );
...
( $ret, $out, $err ) =
  run_script( 'sd', [ "ticket", "update", $pony_id, "--", "status=closed" ] );
like( $out, qr/^Ticket(.*)updated/ );
diag($out);
diag($err);
( $ret, $out, $err ) =
  run_script( 'sd' => [ "ticket", "basics", $pony_id, "--batch" ] );
...
( $ret, $out, $err ) =
  run_script( 'sd', [ 'push', '--to', $sd_trac_url, '--prefer', 'source' ] );
diag($out);
diag($err);
...
is( $closed_ticket->status, 'closed',
    "The ticket is closed after we push to trac" );
