...
sub test_push_of_attachment {
    ...
    my ( $fh, $filename ) =
      File::Temp::tempfile( SUFFIX => '.txt', UNLINK => 1 );
    print $fh "TIMTOWTDI\n";
    close $fh;
    sleep 2;    # to make trac happy
    ( $ret, $out, $err ) =
      run_script( 'sd',
        [ 'ticket', 'attachment', 'create', $yatta_id, '--file', $filename ] );

    ok( !( $? >> 8 ), $err );

    ( $ret, $out, $err ) =
      run_script( 'sd', [ 'push', '--to', $sd_trac_url ] );
    ok( !( $? >> 8 ), $err );
    diag($out);
    diag($err);
...
sub test_push_of_comment {
    ...
    my $tickets = Net::Trac::TicketSearch->new( connection => $trac );
    my $result = $tickets->query( summary => { not => 'nonsense' } );
    like( $tickets->results->[0]->comments->[0]->content,
        qr/The text of the comment./ );
}
