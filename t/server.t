...
eval { require Test::HTTP::Server::Simple }
  || plan skip_all => 'You need Test::HTTP::Server::Simple to run these tests';
eval { require Test::WWW::Mechanize }
  || plan skip_all => 'You need Test::WWW::Mechanize to run these tests';
...
$ua->post_ok( url( 'records', 'ticket', $uuid . ".json" ),
    { status => 'open' } );
...
$ua->submit_form(
    form_number => 2,
    fields      => {
        'prophet-field-function-create-ticket-prop-owner' =>
          'jesse@example.com',
