...
on qr'.' => sub {
    ...
    if ( $result && $result->success ) {
        $self->server->_send_redirect(
            to => '/ticket/' . $result->record_uuid );
    } else {
        next_rule;
    }
};
...
on qr'.' => sub {
    my $self = shift;
    $self->server->page_nav->child(
        history => label => 'History',
        url     => '/history'
    );
    my $tickets = $self->server->page_nav->child(
        tickets => label => 'Tickets',
        url     => '/'
    );
    $tickets->child(
        go => label =>
          '<form method="GET" action="/ticket/"><a href="#">Show ticket # <input type=text name=id size=3></a></form>',
        escape_label => 0
    ) unless ( $self->server->static );

    my $milestones = $tickets->child(
        milestones => label => 'Milestones',
        url        => '/milestones'
    );
    my $items =
      $self->server->app_handle->setting( label => 'milestones' )->get();
    foreach my $item (@$items) {
        my $m = $milestones->child(
            $item => label => $item,
            url   => '/milestone/' . $item
        );
    ...
    my $components = $tickets->child(
        components => label => 'Components',
        url        => '/components'
    );
    my $component_list =
      $self->server->app_handle->setting( label => 'components' )->get();
    foreach my $item (@$component_list) {
        my $c = $components->child(
            $item => label => $item,
            url   => '/component/' . $item
        );
...
under { method => 'POST' } => sub {
    ...
    on qr'^/ticket/([\w\d-]+)/edit$' =>
      sub { shift->server->_send_redirect( to => '/ticket/' . $1 ); };
    on qr'^/(?!records)$' =>
      sub { shift->server->_send_redirect( to => $1 ); };
};
...
under { method => 'GET' } => sub {
    ...
    on qr'^/history/?(\d*)/?$' =>
      sub { my $since = $1; shift->show_template( 'history', $since ) };
    ...
    under qr'^/ticket/' => sub {
        ...
        on qr'^([\w\d-]+)/?' => sub {
            ...
            if ( !$ticket->luid ) {
                $self->server->_send_404()
                  ;    #redirect( to => "/error/ticket_does_not_exist");
            } else {
                next_rule;
            }
        };
        on qr'^([\w\d-]+)/?$' =>
          sub { shift->server->_send_redirect( to => "/ticket/$1/view" ) };
        on qr'^([\w\d-]+)/edit$' =>
          sub { shift->show_template( 'edit_ticket', $1 ) };
        on qr'^([\w\d-]+)/history$' =>
          sub { shift->show_template( 'show_ticket_history', $1 ) };
        on qr'^([\w\d-]+)/view$' =>
          sub { shift->show_template( 'show_ticket', $1 ) };
    };
};
