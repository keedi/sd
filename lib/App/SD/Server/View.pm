...
my @BASIC_PROPS =
  qw(status milestone component owner reporter due created tags description);
...
template '' => page {'Project overview'}
content {
    my $self = shift;
    div {
        { class is 'stats sidebar' };
    ...
    div {
        { class is 'overview' };
        h2 {
            'Your active tickets for '
              . $self->app_handle->setting( label => 'default_milestone' )
              ->get()->[0];
        };
        show('/tickets/hot');
    };
...
template 'all_tickets' => page {'All tickets'}
content {
    shift->show_tickets( sub {1} );
};

template 'milestones' => page {'Project milestones'}
content {
    show 'milestone_list';
};
...
template 'property_list' => sub {
    ...
    $self->find_tickets(
        sub {
            my $ticket = shift;
            return
              if ( $property_name ne 'status' && !$ticket->has_active_status );
        ...
        ul {
            ...
            foreach my $prop (@order) {
                li {
                    ...
                    a {
                        {
                            href is $self->server->make_link_relative(
                                '/' . $property_name . '/' . ( $prop || '' ) )
                        }
                        ( $prop ? $prop : 'None' );
                    }
...
content {
    ...
    $self->show_tickets(
        sub {
            my $item = shift;
            ( ( $item->prop('component') || '' ) eq $component
...
content {
    ...
    $self->show_tickets(
        sub {
            my $item = shift;
            ( ( $item->prop('milestone') || '' ) eq ( $milestone || '' )
                  && $item->has_active_status )
              ? 1
              : 0;
        }
...
template new_ticket => page {'Create a new ticket'}
content {
    my $self = shift;

    form {
        { class is 'create-ticket' };
...
template footer => sub {
    ...
    div {
        id is 'project-versions';
        outs(
            "SD $App::SD::VERSION - Issue tracking for the distributed age - "
...
template header => sub {
    ...
    div {
        class is 'logowrapper';
        img {
            src is '/static/sd/images/sd.png';
            alt is 'SD Logo', class is 'logo';
        };
    };
    div {
        class is 'project-name';
        " for "
          . $self->app_handle->setting( label => 'project_name' )->get()->[0];
    };
    h1 {$title};
};
...
template '/tickets/hot' => sub {
    ...
    my $current_milestone =
      $self->app_handle->setting( label => 'default_milestone' )->get()->[0];
...
template '/tickets/open' => sub {
    my $self = shift;
    $self->show_tickets(
        sub { my $item = shift; return $item->has_active_status ? 1 : 0; } );
...
private template 'ticket_list' => sub {
            ...
            tbody {
                for my $ticket (@$tickets) {
                    row {
                        cell {
                            class is 'id';
                            $self->ticket_link( $ticket => $ticket->luid );
                        };
                        for (
                            qw|status milestone component owner reporter due created|
                          )
                        {
    ...
    script {
        ...
        outs_raw(
            '$("td.created,td.due").prettyDateTag();
...
content {
    ...
    div {
        ...
        show(
            ...
            change_filter   => sub {1},
            sort_changesets => sub {
                sort { $b->sequence_no <=> $a->sequence_no } @_;
            },
            change_header => sub {
                my $change = shift;
...
content {
    ...
    show(
        ...
        change_filter => sub {
            my $change = shift;
            return $ticket->uuid eq $change->record_uuid ? 1 : 0;
        },
    ...
    ul {
        ...
        li {
            a {
                {
                    href is $self->server->make_link_relative(
                        '/ticket/' . $ticket->uuid . '/edit' )
                };
                'Update';
            };
        }
        unless ( $self->server->static );
        li {
            a {
                {
                    href is $self->server->make_link_relative(
                        '/ticket/' . $ticket->uuid . '/history' )
                };
                'History';
            };
        };
...
private template 'ticket_basics' => sub {
    ...
    div {
        ...
        for my $key (
            qw'status component milestone',
            (
                grep { $_ ne 'description' }
    ...
    script {
        outs_raw(
            '$("div.created,div.due").prettyDateTag();
...
private template format_history => sub {
    ...
    my %args = (
        ...
        change_header   => undef,
        sort_changesets => sub {
            sort { $a->created cmp $b->created } @_;
        },
        @_
    );
    ...
    dl {
        { class is 'history' };
        for
          my $changeset ( $args{sort_changesets}->( @{ $args{changesets} } ) )
        {
            dt {
                span {
                    { class is 'created' };
                ...
                for my $change ( $changeset->changes ) {
                    if ( $args{change_filter}->($change) ) {
                        ...
                        ul {
                            ...
                            for (
                                grep {$_}
                                map  { show_history_prop_change($_) }
                                ( $change->prop_changes )
                            );
                        }
                    } else {
                        i {
                            'Something else changed - It was '
                              . $change->record_type . " "
                              . $change->record_uuid;
                        };
                    }
...
sub show_history_prop_change {
    ...
    if ( defined $pc->old_value && defined $pc->new_value ) {
        ...
        . span { class is 'value old'; $pc->old_value }
        . span { class is 'prose';     " to " }
        . span { class is 'value new'; $pc->new_value };
    } elsif ( defined $pc->new_value ) {
        span   { class is 'property';  $pc->name }
        . span { class is 'prose';     ' set to ' }
        . span { class is 'value new'; $pc->new_value }

    } elsif ( defined $pc->new_value ) {
        span     { class is 'property';  $pc->name } . ' '
          . span { class is 'value old'; $pc->new_value }
        . span { class is 'prose'; ' deleted' };
    }
}
...
template ticket_comments => sub {
    ...
    my $ticket = shift;
    my @comments =
      sort { $a->prop('created') cmp $b->prop('created') }
      @{ $ticket->comments };
    if (@comments) {
        h2 {
            { class is 'conmments' };
...
template ticket_comment => sub {
    ...
    li {
        ...
        div {
            ...
            } elsif ( $comment->prop('content_type')
                && $comment->prop('content_type') =~ m{text/html}i )
            {
